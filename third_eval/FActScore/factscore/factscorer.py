...
class FactScorer(object):
    ...
    def __init__(self,
        ...
        if "llama" in model_name:
            ...
            self.lm = OpenAIModel("gpt-5", key_path=openai_key)
        else:
            self.lm = None
    ...
    def get_score(self,
        ...
        if atomic_facts is not None:
            ...
            for topic, gen in zip(topics, generations):
                ...
                response_abstained = response_abstained or is_non_answer(gen)
...
if __name__ == '__main__':
    ...
    wandb.login()
    run = wandb.init(project=args.project_id, config=args, job_type='evaluation')

    table_id = f"run-{args.table_id}-generation:v0"
    generation_table = run.use_artifact(f"tree-lli/{args.project_id}/{table_id}", type='run_table')
    generation_dir = generation_table.download()
    generation_dir = f"artifacts/{table_id}"
    generation_path = os.path.join(generation_dir, "generation.table.json")
    ...
    if not os.path.isfile(generation_path):
        print(f"Abort! the file not found: {generation_path}.")

    with open(generation_path, 'r', encoding="utf-8") as f:
        rows = [json.loads(_f) for _f in f][0]['data']
        for i, (query, generation) in enumerate(rows):
            topics.append(query[14:-1])
            print(topics[-1])
            if generation is None: generation = ''
    ...
    wandb.log({
        "eval/score": 100*out["score"],
        "eval/score w/o length penalty": 100*out["init_score"],
        "eval/respond ratio": 100*out["respond_ratio"],
        "eval/facts per response": out["num_facts_per_response"]
    })
